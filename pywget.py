...
# @Last Modified time: 2019-09-14 13:12:53
...
class pywget:
    ...
    def __init__(self, config={}):
        self._block = int(config['block'] if 'block' in config else 1024)
        self._proxy = config['proxy']
        self._sock = self.__server_Connect__(config['proxy']) if config['proxy'] else False
    ...
    def __do_recv__(self, headers):
        ...
        # 0) 发送网址信息
        print('[send-url]%s[%s]' % (url, getpass.getuser()))
        self.__mysend__(('[send-url]%s[%s]' % (url, getpass.getuser())).encode())
        ...
        if int(msg[0]):
            headers_range = self.__support_continue_do__(True)
            # print(headers_range)
            headers.update(headers_range)
        if msg[1:] == b'[start]':
            print('排队结束, 下载中...')
        else:
            print(msg)
        ...
        # 2) 发送网页请求头
        self.__mysend__(json.dumps(headers).encode('utf-8'))
    ...
    def download(self):
        ...
        url, filename, force, block = self.url, self.filename, self.force, self._block
        if not filename:
